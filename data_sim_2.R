...
init_age <- as.integer(runif(n=population_size, min=25, max=100))
...
for (time_point in 1:time_max){
  ...
  if((length(curr_pop$indiv_ID)==0)) {
    ...
    if(time_point%%output_timept == 0){ 
      ...
      # Plots
      print(ggplot() + geom_histogram(data=data.frame(curr_pop_end), aes(x=age, y = after_stat(density)), bins=30) + theme_bw() + labs(title="Age count"))
      print(ggplot() + geom_point(data=data.frame(curr_pop_end), aes(x=age, y=MR), bins=30) + theme_bw() + labs(title="MR by age"))
...
#plot_liveage <- ggplot(age_df, aes(x=time, y=mean_age, group=time)) + geom_boxplot() + labs(title="Live age")
plot_liveage  <- ggplot() + 
