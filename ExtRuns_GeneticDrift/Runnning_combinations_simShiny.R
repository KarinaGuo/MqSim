...
## Load in libraries
library(tidyverse)
output_logfile = paste0("ExtRuns_GeneticDrift/run_log_", Sys.Date(),".txt")
sink(output_logfile)

for (pop_size in c(10, 100, 500, 1000, 5000, 10000)){
  ...
  #rm(list = ls())
  ## Load in parameters
  source("ExtRuns_GeneticDrift/configurations_fromMultRun")
...
################# Plotting results
library(tidyverse)
live_size_df = NULL; MR_df=NULL
for (pop_size in c(100, 500, 1000, 5000, 10000)){
  print(pop_size)
  population_size = pop_size
  population_carrying_capacity = population_size*5 # If population creeps above this value, comp is used to punish the population size by increasing the minimum probability of death based on how much higher population size is than the carrying capacity
  population_minimum_size = population_size/10 # If population falls below this value, there is a 10x increased chance of recruitment
...
plot_livesize <- ggplot() + 
  ...
  theme_bw() +
  facet_wrap(~pop_size_set, scales = "free") +
  labs(title="Live population size") 

plot_liveMR_facet <- ggplot() +
...
plot_liveMR <- ggplot() +
  geom_point(data=MR_df, aes(x=time, y = MR_mean_summ, colour=as.character(pop_size_set), shape=as.character(pop_size_set)), alpha=0.7) +
  #geom_errorbar(data=MR_df, aes(x=time, ymax = MR_mean_summ + MR_sd_summ, ymin = MR_mean_summ - MR_sd_summ, colour=as.character(pop_size_set))) + 
  stat_smooth(data=MR_df, aes(x=time, y = MR_mean_summ, group=as.character(pop_size_set) , colour=as.character(pop_size_set)), linewidth = 0.75, linetype="dashed", span=10) +
  geom_vline(xintercept=MR_timepoint, linewidth = 0.75, linetype="dashed", colour="chocolate") +
  ggforce::facet_zoom(xlim=c(1100,1200)) +
  theme_bw() +
  labs(title="Live MR")
