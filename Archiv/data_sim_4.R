...
for (time_point in 1:time_max){
  ...
  # } else{
    ...
    curr_pop_alive <- list(indiv_ID=curr_pop$indiv_ID[!as.logical(indiv_death)], age=curr_pop$age[!as.logical(indiv_death)], MR=curr_pop$MR[!as.logical(indiv_death)], time=curr_pop$time[!as.logical(indiv_death)])
    ...
    if (time_point == intercept_timepoint & intercept_togg){
      ...
      cat("Time at:", time_point,"\n",
          "Individuals alive:", length(curr_pop_alive$indiv_ID), "\n",
          "Mean MR of live individuals:", mean(curr_pop_alive$MR), "\n")
      ...
      curr_pop_og <- curr_pop_alive
      curr_pop_int <- list(
        indiv_ID=c(curr_pop_alive$indiv_ID, intercept_pop$indiv_ID), 
        age=c(curr_pop_alive$age, intercept_pop$age), 
        MR=c(curr_pop_alive$MR, intercept_pop$MR), 
        time=c(curr_pop_alive$time, intercept_pop$time))
      
      curr_pop_alive <- curr_pop_int
      
      indiv_count_end=length(curr_pop_alive$indiv_ID) + indiv_count_start
      indiv_death = c(indiv_death, rep(0, intercept_indiv)) 
      
      cat(  "## Intervention ##\n",
            "Individuals alive:", length(curr_pop_alive$indiv_ID), "\n",
            "Mean MR of live individuals after:", mean(curr_pop_alive$MR), "\n",
            "#################\n")
      
      # Plots
      print(ggplot() + geom_point(data=data.frame(curr_pop_alive), aes(x=age, y=MR)) + theme_bw() + labs(title=paste("MR by age at", time_point)))
    } else {
      indiv_count_end = indiv_count_start
    }
    
    indiv_alive_count = length(curr_pop_alive$indiv_ID)
    ...
    indiv_diff_torec = population_size-indiv_alive_count; if(indiv_diff_torec < 0){indiv_diff_torec=0}
    
    uniq_befrec_size <- data.frame(time=time_point, sum_size=length(unique(curr_pop_alive$indiv_ID)))
    uniq_befrec_size_df <- rbind(uniq_befrec_size_df, uniq_befrec_size) 
    
    
    if (indiv_count_end >0){
      curr_pop_alive <- recruit_rate(pop=curr_pop_alive, recruit_size=indiv_diff_torec, recruitment_age=recruitment_age, pop_size=population_size, recruitment_size_mean=recruitment_mean, recruitment_size_sd=recruitment_sd, recruitment_constant=recruitment_const, MR_togg=MR_rec_toggle, MR_recruit_impact_val=MR_recruit_impact_tp, MR_rec_adjusted=MR_rec_adj, age_togg=age_rec_toggle, age_recruit_impact_val=age_recruit_impact_value, rec_age_shiftch=rec_age_shift)
      
      recruited_indivs = length(curr_pop_alive$indiv_ID) - indiv_alive_count
      indiv_death = c(indiv_death, rep(0, recruited_indivs)) 
      
      indiv_count_end = length(curr_pop_alive$indiv_ID) + indiv_count_end
    } else {
      indiv_count_end = indiv_count_start
    }
    ...
    curr_pop_end <- list(indiv_ID=curr_pop_alive$indiv_ID, age=curr_pop_alive$age+1, MR=curr_pop_alive$MR, time=curr_pop_alive$time+1)
...
plot_livesize <- ggplot() + 
  geom_point(data=uniq_befrec_size_df, aes(x=time, y=sum_size)) + 
  stat_smooth(data=uniq_befrec_size_df, aes(x=time, y = sum_size), linewidth = 0.85, linetype="dashed", colour="grey40", span=10) +
