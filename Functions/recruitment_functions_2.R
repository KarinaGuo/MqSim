...
##############################################
recruit_rate <- function(pop, population_min_size, recruitment_age, recruitment_size_mean, recruitment_size_sd, recruitment_constant, age_togg, age_recruit_impact_val, MR_togg, MR_recruit_impact_val, MR_rec_adjusted, rec_age_shiftch){
  ...
  MR_impact <- 1 / (1 + MR_recruit_impact_val ^ recruitment_indivs_MR )
  age_impact <- (recruitment_indivs_ages / rec_age_shiftch) ^ age_recruit_impact_val
  
  # Both MR and age impacts
  if (MR_togg & age_togg) {
    total_impact <- MR_impact + age_impact - (MR_impact * age_impact)  # inclusion-exclusion style
