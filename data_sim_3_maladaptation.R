# running data_sim_3 with maladaptation
...
for (time_point in 1:time_max){
  ...
  if((length(curr_pop_start_pop1$indiv_ID)==0) | (length(curr_pop_start_pop2$indiv_ID)==0)) {
    ...
    indiv_alive_count_pop1=length(curr_pop_start_pop1$indiv_ID)
    
    indiv_count_start_pop2=length(curr_pop_start_pop2$indiv_ID) + indiv_count_end_pop2
    indiv_alive_count_pop2=length(curr_pop_start_pop2$indiv_ID)
    ...
    recruited_indivs_pop2 = length(curr_pop_recruited_pop2 $indiv_ID) - indiv_alive_count_pop2
    indiv_count_end_pop2 = recruited_indivs_pop2 + indiv_count_end_pop2
    
    ## Mortality
    
    if (time_point>=MR_timepoint & MR_lateintro){
      indiv_death_pop1 <- mortality_death_rate_MRlate(pop=curr_pop_start_pop1, population_capacity=population_carrying_capacity, comp_togg=comp_imp, comp_impact_val=comp_impact, MR_death_impact_val=MR_death_impact_pop1, MR_age_impact_val=MR_age_impact_pop1, age_impact_val=age_impact_pop1, mortality_age_shiftch=mortality_age_shift, MR_intro=MR_lateintro, MR_intro_timepoint=MR_timepoint, int_togg=int_togg, intercept_pop_indiv_ID=intercept_pop_indiv_ID)
      ...
      indiv_death_pop1 <- mortality_death_rate(pop=curr_pop_start_pop1, population_capacity=population_carrying_capacity, comp_togg=comp_imp, comp_impact_val=comp_impact, MR_togg=MR_imp, MR_death_impact_val=MR_death_impact_beforeintro, MR_age_impact_val=MR_age_impact_pop1, age_impact_val=age_impact_pop1, mortality_age_shiftch=mortality_age_shift, int_togg=int_togg, intercept_pop_indiv_ID=intercept_pop_indiv_ID)
      if(time_point==1){cat("Using mortality_death_rate & MR before imp \n")}
      
    } else if (!MR_lateintro | !MR_imp) {
      indiv_death_pop1 <- mortality_death_rate(pop=curr_pop_start_pop1, population_capacity=population_carrying_capacity, comp_togg=comp_imp, comp_impact_val=comp_impact, MR_togg=MR_imp, MR_death_impact_val=MR_death_impact_pop1, MR_age_impact_val=MR_age_impact_pop1, age_impact_val=age_impact_pop1, mortality_age_shiftch=mortality_age_shift, int_togg=int_togg, intercept_pop_indiv_ID=intercept_pop_indiv_ID)
      if(time_point==1){cat("Using mortality_death_rate \n")}
    }
    
    if (time_point>=MR_timepoint & MR_lateintro){
      indiv_death_pop2 <- mortality_death_rate_MRlate(pop=curr_pop_start_pop2, population_capacity=population_carrying_capacity, comp_togg=comp_imp, comp_impact_val=comp_impact, MR_death_impact_val=MR_death_impact_pop2, MR_age_impact_val=MR_age_impact_pop2, age_impact_val=age_impact_pop2, mortality_age_shiftch=mortality_age_shift, MR_intro=MR_lateintro, MR_intro_timepoint=MR_timepoint, int_togg=int_togg, intercept_pop_indiv_ID=intercept_pop_indiv_ID)
      if(time_point==MR_timepoint){cat("Using mortality_death_rate_MRlate \n")}
    } else if (time_point<MR_timepoint & MR_lateintro){
      MR_death_impact_beforeintro=0
      indiv_death_pop2 <- mortality_death_rate(pop=curr_pop_start_pop2, population_capacity=population_carrying_capacity, comp_togg=comp_imp, comp_impact_val=comp_impact, MR_togg=MR_imp, MR_death_impact_val=MR_death_impact_beforeintro, MR_age_impact_val=MR_age_impact_pop2, age_impact_val=age_impact_pop2, mortality_age_shiftch=mortality_age_shift, int_togg=int_togg, intercept_pop_indiv_ID=intercept_pop_indiv_ID)
      if(time_point==1){cat("Using mortality_death_rate & MR before imp \n")}
    } else if (!MR_lateintro) {
      indiv_death_pop2 <- mortality_death_rate(pop=curr_pop_start_pop2, population_capacity=population_carrying_capacity, comp_togg=comp_imp, comp_impact_val=comp_impact, MR_togg=MR_imp, MR_death_impact_val=MR_death_impact_pop2, MR_age_impact_val=MR_age_impact_pop2, age_impact_val=age_impact_pop2, mortality_age_shiftch=mortality_age_shift, int_togg=int_togg, intercept_pop_indiv_ID=intercept_pop_indiv_ID)
      if(time_point==1){cat("Using mortality_death_rate \n")}
    }
    
    
    death_df_curr_pop1 <- data.frame(Dead_ID=curr_pop_start_pop1$indiv_ID[as.logical(indiv_death_pop1)], age=curr_pop_start_pop1$age[as.logical(indiv_death_pop1)], MR=curr_pop_start_pop1$MR[as.logical(indiv_death_pop1)], time=curr_pop_start_pop1$time[as.logical(indiv_death_pop1)])
    death_df_pop1 <- rbind(death_df_pop1, death_df_curr_pop1)
    
    death_df_curr_pop2 <- data.frame(Dead_ID=curr_pop_start_pop2$indiv_ID[as.logical(indiv_death_pop2)], age=curr_pop_start_pop2$age[as.logical(indiv_death_pop2)], MR=curr_pop_start_pop2$MR[as.logical(indiv_death_pop2)], time=curr_pop_start_pop2$time[as.logical(indiv_death_pop2)])
    death_df_pop2 <- rbind(death_df_pop2, death_df_curr_pop1)
    
    indiv_death_pop1 = c(indiv_death_pop1, rep(0, recruited_indivs_pop1)) 
    indiv_death_pop2 = c(indiv_death_pop2, rep(0, recruited_indivs_pop2)) 
    
    #### Final time point pop
    curr_pop_end_pop1 <- list(indiv_ID=curr_pop_recruited_pop1$indiv_ID[!as.logical(indiv_death_pop1)], age=curr_pop_recruited_pop1$age[!as.logical(indiv_death_pop1)]+1, MR=curr_pop_recruited_pop1$MR[!as.logical(indiv_death_pop1)], time=curr_pop_recruited_pop1$time[!as.logical(indiv_death_pop1)]+1)
    curr_pop_end_pop2 <- list(indiv_ID=curr_pop_recruited_pop2$indiv_ID[!as.logical(indiv_death_pop2)], age=curr_pop_recruited_pop2$age[!as.logical(indiv_death_pop2)]+1, MR=curr_pop_recruited_pop2$MR[!as.logical(indiv_death_pop2)], time=curr_pop_recruited_pop2$time[!as.logical(indiv_death_pop2)]+1)
    
    
    # Generate summary data if not dead
    if (length(curr_pop_recruited_pop1$indiv_ID)>sum(indiv_death_pop1)){ 
      MR_summ_pop1 <- data.frame(time=time_point, MR_mean_summ=mean(curr_pop_recruited_pop1$MR[!as.logical(indiv_death_pop1)], na.rm=TRUE), MR_sd_summ=sd(curr_pop_recruited_pop1$MR[!as.logical(indiv_death_pop1)]), maladaptation_time=maladaptation_chance_pop1)
      MR_df_pop1 <- rbind(MR_df_pop1, MR_summ_pop1)
      
      age_summ_pop1 <- data.frame(time=time_point, age_mean_summ=mean(curr_pop_recruited_pop1$age[!as.logical(indiv_death_pop1)]), age_sd_summ=sd(curr_pop_recruited_pop1$age[!as.logical(indiv_death_pop1)]), pop_size=length(curr_pop_recruited_pop1$indiv_ID))
      age_df_pop1 <- rbind(age_df_pop1, age_summ_pop1) 
    }
    
    if (length(curr_pop_recruited_pop2$indiv_ID)>sum(indiv_death_pop2)){ 
      MR_summ_pop2 <- data.frame(time=time_point, MR_mean_summ=mean(curr_pop_recruited_pop2$MR[!as.logical(indiv_death_pop2)], na.rm=TRUE), MR_sd_summ=sd(curr_pop_recruited_pop2$MR[!as.logical(indiv_death_pop2)]), maladaptation_time=maladaptation_chance_pop2)
      MR_df_pop2 <- rbind(MR_df_pop2, MR_summ_pop2)
      
      age_summ_pop2 <- data.frame(time=time_point, age_mean_summ=mean(curr_pop_recruited_pop2$age[!as.logical(indiv_death_pop2)]), age_sd_summ=sd(curr_pop_recruited_pop2$age[!as.logical(indiv_death_pop2)]), pop_size=length(curr_pop_recruited_pop2$indiv_ID))
      age_df_pop2 <- rbind(age_df_pop2, age_summ_pop2) 
    }
    
    live_size_pop1 <- data.frame(time=time_point, sum_size=length(curr_pop_end_pop1$indiv_ID[!as.logical(indiv_death_pop1)]))
    live_size_df_pop1 <- rbind(live_size_df_pop1, live_size_pop1) 
    
    live_size_pop2 <- data.frame(time=time_point, sum_size=length(curr_pop_end_pop2$indiv_ID[!as.logical(indiv_death_pop2)]))
...
write.csv(live_size_df_pop1, file="Maladaptation/SIZE_MaladaptationRunPop1.csv")
write.csv(live_size_df_pop2, file="Maladaptation/SIZE_MaladaptationRunPop2.csv")
write.csv(MR_df_pop2, file="Maladaptation/MR_MaladaptationRunPop2.csv")
write.csv(MR_df_pop1, file="Maladaptation/MR_MaladaptationRunPop1.csv")

# Plot both runs on top
Size_MaladaptationRunPop1 <- read.csv("Maladaptation/SIZE_MaladaptationRunPop1.csv"); Size_MaladaptationRunPop1$Run <- "MaladaptationRunPop1"
Size_MaladaptationRunPop2 <- read.csv("Maladaptation/SIZE_MaladaptationRunPop2.csv"); Size_MaladaptationRunPop2$Run <- "MaladaptationRunPop2"
Size_SoloRunPop1 <- read.csv("MR_tests/SIZE_base_0.2_Intro1000.csv"); Size_SoloRunPop1$Run <- "SoloRunPop1"
Size_SoloRunPop2 <- read.csv("MR_tests/SIZE_base_0.9_Intro1000.csv"); Size_SoloRunPop2$Run <- "SoloRunPop2"
...
Size_Pop1_plot <- ggplot() + 
  geom_point(data=Size_Pop1_res, aes(x=time,  y=sum_size, colour=Run), alpha = 0.3, size=0.2) +
  stat_smooth(data=Size_Pop1_res, aes(x=time, y = sum_size, group=Run, colour=Run), linewidth = 1, linetype="dashed", span=0.01)  +
...
Size_Pop2_plot <- ggplot() + 
  geom_point(data=Size_Pop2_res, aes(x=time,  y=sum_size, colour=Run), alpha = 0.3, size=0.2) +
  stat_smooth(data=Size_Pop2_res, aes(x=time, y = sum_size, group=Run, colour=Run), linewidth = 1, linetype="dashed", span=0.01) +
...
##
MR_MaladaptationRunPop1 <- read.csv("Maladaptation/MR_MaladaptationRunPop1.csv"); MR_MaladaptationRunPop1$Run <- "MaladaptationRunPop1"
MR_MaladaptationRunPop2 <- read.csv("Maladaptation/MR_MaladaptationRunPop2.csv"); MR_MaladaptationRunPop2$Run <- "MaladaptationRunPop2"
MR_SoloRunPop1 <- read.csv("MR_tests/MR_base_0.2_Intro1000.csv"); MR_SoloRunPop1$Run <- "SoloRunPop1"; MR_SoloRunPop1$maladaptation_time <- 0
MR_SoloRunPop2 <- read.csv("MR_tests/MR_base_0.9_Intro1000.csv"); MR_SoloRunPop2$Run <- "SoloRunPop2"; MR_SoloRunPop2$maladaptation_time <- 0
...
MR_Pop1_plot <- ggplot() + 
  geom_point(data=MR_Pop1_res, aes(x=time,  y=MR_mean_summ, colour=Run, shape=as.character(maladaptation_time)), alpha = 0.3, size=0.2) +
  geom_point(data=(MR_Pop1_res %>% filter(maladaptation_time==1)), aes(x=time,  y=MR_mean_summ, colour=Run), shape=17, alpha = 0.5, size=0.5, colour="darkolivegreen") +
  stat_smooth(data=MR_Pop1_res, aes(x=time, y = MR_mean_summ, group=Run, colour=Run), linewidth = 1, linetype="dashed", span=10) +
  ylim(c(0,1)) +
  labs(title="Population 1 - MR=0.2 (weak)", y="Mean myrtle rust", x="Timepoint") +
  scale_colour_manual(values=c("MaladaptationRunPop1"="seagreen", "SoloRunPop1"="powderblue")) +
  geom_vline(xintercept=MR_timepoint, linewidth = 0.75, linetype="dashed", colour="chocolate", alpha=0.5) +
  theme_bw()

MR_Pop2_plot <- ggplot() + 
  geom_point(data=MR_Pop2_res, aes(x=time,  y=MR_mean_summ, colour=Run), alpha = 0.3, size=0.2) +
  geom_point(data=(MR_Pop2_res %>% filter(maladaptation_time==1)), aes(x=time,  y=MR_mean_summ, colour=Run), shape=17, alpha = 0.5, size=0.5, colour="red3") +
  stat_smooth(data=MR_Pop2_res, aes(x=time, y = MR_mean_summ, group=Run, colour=Run), linewidth = 1, linetype="dashed", span=10) +
  ylim(c(0,1)) +
  labs(title="Population 2 - MR=0.9 (strong)", y="Mean myrtle rust", x="Timepoint") +
  scale_colour_manual(values=c("MaladaptationRunPop2"="tomato3", "SoloRunPop2"="lightsalmon")) +
  geom_vline(xintercept=MR_timepoint, linewidth = 0.75, linetype="dashed", colour="chocolate", alpha=0.5) +
  theme_bw()  
...
ggsave(Size_Pop1_plot, file="Plots/Maladaptation_size_plot_Pop1.jpg", limitsize = F, width=2000, height=2500, units='px')
ggsave(Size_Pop2_plot, file="Plots/Maladaptation_size_plot_Pop2.jpg", limitsize = F, width=2000, height=2500, units='px')
ggsave(MR_plot, file="Plots/Maladaptation_MR_plot.jpg", limitsize = F, width=4000, height=2500, units='px')
